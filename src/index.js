...
function loadLessions(dataDir, course) {
  ...
  const lessonList = getFsList(dir, { onlyFiles: true }).map(
    ({ id, name, position }) => {
      const completeLession = require(path.join(dir, `${position}-${id}.js`));
      return Object.assign(
        { id, position, courseId: course.id },
        completeLession
      );
    }
  );
...
function loadLessionsWithSubLessons(dataDir, course) {
  try {
    ...
    lessions.forEach(objLession => {
      ...
      var subLessionList = getFsList(
        path.join(absoluteCourseDir, lessionDirName),
        { onlyFiles: true }
      );
    ...
    default:
      if (item instanceof Array) {
        ...
        const content = Object.keys(item)
          .map(k => {
            return `${spaces(indent + 2)}${k}: ${stringify(
              item[k],
              indent + 2
            )}`;
          })
